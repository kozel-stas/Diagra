...
public class IEController {
    ...
    public ResponseEntity<IEResponse> convert(@RequestBody @Valid IERequest request) throws IOException {
        TransitionService.Resource resource = transitionService.load(TransitionUtil.subTransitionLink(request.getTransitionLink()));
        if (resource == null) {
        ...
        String link = transitionService.generateTransitionLink(new TransitionService.Resource(converted.getResourceType().getMediaType(), converted.getResourceType().toResource(converted.getObject())));
